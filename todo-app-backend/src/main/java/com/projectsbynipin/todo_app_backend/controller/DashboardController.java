...
@Tag(
        name = "Dashboard",
        description = "APIs related to dashboard and dashboard columns"
)
@RestController
...
public class DashboardController {
    ...
    @Operation(
            summary = "Edit dashboard column name",
            description = "Edit the name of the dashboard column",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    @PutMapping(path = "/edit-dashboard-column-name")
    public ResponseEntity<ApiResponse<Void>> editDashboardColumnName(@Valid @RequestBody EditDashboardColumnNameRequestDto editDashboardColumnNameRequestDto, @AuthenticationPrincipal UserInfoDetails userInfoDetails) {
        ApiResponse<Void> apiResponse = dashboardColumnService.editDashboardColumnName(editDashboardColumnNameRequestDto, userInfoDetails);
        return new ResponseEntity<>(apiResponse, HttpStatus.OK);
    }
